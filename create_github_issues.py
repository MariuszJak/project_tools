...
def verify_repo_access(token: str) -> bool:
    ...
    if response.status_code == 200:
        ...
        if is_private:
            print("✓ Repozytorium jest prywatne - dostęp potwierdzony")
        else:
            print("✓ Repozytorium jest publiczne - dostęp potwierdzony")
        return True
        ...
        print(
            f"Błąd: Repozytorium {REPO_OWNER}/{REPO_NAME} nie istnieje lub nie masz do niego dostępu."
        )
        print("\nMożliwe przyczyny:")
...
def create_issue(token: str, title: str, body: str) -> dict:
    ...
    if response.status_code == 201:
        ...
        if labels_response.status_code not in [200, 201]:
            print("  ⚠️  Uwaga: Nie udało się dodać etykiet (może nie istnieją w repozytorium)")

        return issue_data
    elif response.status_code == 403:
        print("Błąd 403: Brak uprawnień do tworzenia issues")
        error_data = response.json() if response.text else {}
        error_msg = error_data.get("message", response.text)
        print(f"Wiadomość: {error_msg}")
        print("\n💡 Rozwiązanie:")
...
def main():
    ...
    for i, story in enumerate(stories, 1):
        ...
        if result:
            ...
            print("  ✗ Nie udało się utworzyć issue")
            failed += 1
        print()
