...
async def test_filter_by_price_total_min(test_client, sample_listings):
    ...
    filter_request = FilterRequest(price_total=PriceRange(min=Decimal("400000.00"), max=None))

    response = await test_client.post(
        "/filters/search", json=filter_request.model_dump(mode="json")
    )
...
async def test_filter_by_price_total_max(test_client, sample_listings):
    ...
    filter_request = FilterRequest(price_total=PriceRange(min=None, max=Decimal("400000.00")))
...
async def test_filter_by_price_per_sqm_min(test_client, sample_listings):
    ...
    filter_request = FilterRequest(price_per_sqm=PriceRange(min=Decimal("8000.00"), max=None))
...
async def test_filter_by_price_per_sqm_max(test_client, sample_listings):
    ...
    filter_request = FilterRequest(price_per_sqm=PriceRange(min=None, max=Decimal("7500.00")))
...
async def test_filter_by_city_and_district(test_client, sample_listings):
    ...
    filter_request = FilterRequest(city="Warszawa", city_district="Centrum")
...
async def test_filter_combined_price_and_rooms(test_client, sample_listings):
    ...
    filter_request = FilterRequest(
        price_total=PriceRange(min=Decimal("400000.00"), max=Decimal("600000.00")), rooms=[3]
    )
...
async def test_filter_no_results(test_client, sample_listings):
    ...
    filter_request = FilterRequest(price_total=PriceRange(min=Decimal("1000000.00"), max=None))
...
async def test_filter_price_per_sqm_with_zero_area(test_client, invalid_listings):
    ...
    for listing in data["listings"]:
        ...
        area = listing.get("area")
        is_zero_area = (
            area is None or area == 0 or (area is not None and Decimal(str(area)) == Decimal("0"))
        )
        if is_zero_area:
            price_sqm = listing.get("price_sqm_zl") or listing.get("price_per_sqm_detailed")
...
async def test_invalid_listings_in_empty_filter(test_client, invalid_listings):
    ...
    [listing["listing_id"] for listing in data["listings"]]
...
async def test_listing_with_missing_price_structure(test_client, invalid_listings):
    ...
    if listing_with_missing_price:
        ...
        assert (
            listing_with_missing_price.get("price_sqm_zl") is None
            or listing_with_missing_price["price_sqm_zl"] is None
        )
        assert (
            listing_with_missing_price.get("price_per_sqm_detailed") is None
            or listing_with_missing_price["price_per_sqm_detailed"] is None
        )
...
async def test_listing_with_zero_area_structure(test_client, invalid_listings):
    ...
    if listing_with_zero_area:
        ...
        assert (
            listing_with_zero_area.get("price_sqm_zl") is None
            or listing_with_zero_area["price_sqm_zl"] is None
        )
        assert (
            listing_with_zero_area.get("price_per_sqm_detailed") is None
            or listing_with_zero_area["price_per_sqm_detailed"] is None
        )
