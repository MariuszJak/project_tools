...
def extract_city_from_locality(locality: Optional[str]) -> Optional[str]:
    ...
    if not locality:
        return None
    cleaned = str(locality).strip()
    # If locality starts with a known city name, extract it
    # For now, all entries seem to be in Warszawa
    if cleaned.startswith("Warszawa"):
        return "Warszawa"
    # Try to extract first word as city
    parts = cleaned.split()
    if parts:
        return parts[0]
    return None
